...
DIR_RESULTADOS = "late_fusion/"
TIPOS = ["genero", "faixa", "faixa_genero"]
METODOS = ["soma", "mult", "max", "min"]
...
def calcular(diretorio, descritor, solver, tipo, arq_folds, metodo, arq):
    ...
    for f in range(len(arq_folds)):
        file = open(diretorio+arq_folds[f] + "/" + metodo + ".csv", "r")
        print(file.name)
    ...
    for l in range(1, len(folds[0]) - 1):
        for f in range(len(folds)):
            ...
            linha = folds[f][l].replace("[", "").replace("]", "").replace("'", "").split(",")
    ...
    arq.write(descritor + "," + tipo + "," + solver + "," + metodo + "," + str(acc_media) + "," + str(desvio_padrao) + ","
              + str(macrof) + "," + str(melhorFold) + "," + str(melhorAcc) + "\n")




def initResultados(dir_resultados, tipos, metodos):

    arq = open(dir_resultados + "resultados.csv", 'w')
    arq.write("Descritor, Classificacao, Solver, Metodo, Acc. Media, Desvio Padrao, Macro-F Media, Melhor Fold, Melhor Acc.\n")
    ...
    for d in range(qtd_descritores):
        ...
        for t in range(qtd_tipos):
            ...
            for s in range(qtd_solvers):
                ...
                for m in range(len(metodos)):
                    calcular(dir_folds, descritores[d], solvers[s], tipos[t], folds, metodos[m], arq)

    arq.close()

initResultados(DIR_RESULTADOS, TIPOS, METODOS)
